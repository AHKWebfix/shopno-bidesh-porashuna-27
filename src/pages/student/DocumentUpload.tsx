      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-white rounded-xl shadow-lg p-4 sm:p-6 lg:p-8">
          ...
          <div className="text-center mb-6 lg:mb-8">
            ...
            <h1 className="text-lg sm:text-xl lg:text-2xl font-bold text-gray-900 mb-2">Document Upload Portal</h1>
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-3 sm:p-4 lg:p-6 mb-4 sm:mb-6">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-3 lg:space-y-0">
            <div>
              <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900">Document Upload</h1>
              <p className="text-gray-600 mt-1 text-sm lg:text-base">Upload your required documents for study abroad application</p>
              {linkId && (
                <p className="text-xs text-gray-500 mt-1">Upload ID: {linkId}</p>
              )}
            </div>
        ...
        {/* Upload Instructions */}
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-3 sm:p-4 lg:p-6 mb-4 sm:mb-6">
          <h2 className="text-base sm:text-lg font-semibold text-gray-900 mb-3 sm:mb-4">Upload Instructions</h2>
        ...
        <div className="space-y-4 sm:space-y-6">
          {documentTypes.map((docType) => (
            <div key={docType.id} className="bg-white rounded-xl shadow-sm border border-gray-200 p-3 sm:p-4 lg:p-6">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-3 sm:mb-4 space-y-2 sm:space-y-0">
                <div className="flex items-center space-x-3">
                  <FileText className="w-4 h-4 sm:w-5 sm:h-5 text-gray-400 flex-shrink-0" />
                  <div>
                    <h3 className="font-medium text-gray-900 text-sm sm:text-base">{docType.label}</h3>
                    {docType.required && (
              ...
              <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 sm:p-6 text-center hover:border-primary transition-colors">
                ...
                <p className="text-xs sm:text-sm text-gray-500">PDF, JPG, PNG up to 10MB</p>
              </div>
              ...
              {uploadedFiles.filter(file => file.type === docType.id).length > 0 && (
                <div className="mt-3 sm:mt-4 space-y-2">
                  ...
                  {uploadedFiles.filter(file => file.type === docType.id).map((file) => (
                    <div key={file.id} className="flex items-center justify-between p-2 sm:p-3 bg-gray-50 rounded-lg">
