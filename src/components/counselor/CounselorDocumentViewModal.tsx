...
const CounselorDocumentViewModal: React.FC<CounselorDocumentViewModalProps> = ({ 
  ...
  const handleViewDocument = (docName: string) => {
    ...
    const simulatedUrl = `https://example.com/documents/${document.studentId}/${docName.toLowerCase().replace(/\s+/g, '-')}.pdf`;
    window.open(simulatedUrl, '_blank');
    console.log('Opening document:', docName, 'for student:', document.studentName);
  };
      ...
      <DialogContent className="sm:max-w-lg max-h-[80vh] overflow-hidden flex flex-col">
        ...
        <div className="overflow-y-auto flex-1 space-y-4 pr-2">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="font-semibold text-lg">{document.studentName}</h3>
              <p className="text-sm text-gray-500">Student ID: #{document.studentId}</p>
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <p className="text-sm text-gray-600">
              Progress: {document.completedDocuments}/{document.totalDocuments} documents
            </p>
          </div>
          ...
          <div className="w-full bg-gray-200 rounded-full h-3">
            <div 
              ...
              style={{ width: `${(document.completedDocuments / document.totalDocuments) * 100}%` }}
            ></div>
          </div>
